...
import { MessageSquare, Repeat2, Heart, BarChart3 } from "lucide-react";
...
export default function PostCard({ post }: Props) {
    ...
    const repostsToShow = isClient ? Math.max(0, stored.reposts) : post.stats.reposts;
    ...
    const onToggleRepost = () =>
        setStored((s) => {
            ...
            return { ...s, reposted, reposts: Math.max(0, s.reposts + (reposted ? 1 : -1)) };
        });
        ...
        <article className="bg-black p-4 text-zinc-200">
            <div className="mb-2 flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 whitespace-pre-wrap text-[15px] leading-6">{post.content}</p>
                </div>
            </div>
                    ...
                    >
                        <Repeat2
                            className={`h-4 w-4 transition-colors ${
                                isClient && stored.reposted ? "text-emerald-400" : "text-zinc-500"
                    ...
                    >
                        ...
                        <ul className="space-y-3">
                            ...
                            {commentsArr.length === 0 && (
                                <div className="text-sm text-zinc-500">Be the first to reply.</div>
                            )}
                        </ul>
                    </motion.div>
                )}
