...
export default function GifPicker({ onPick, fetchGifs = defaultFetchGifs }: Props) {
    ...
    return (
        <div className="relative" ref={ref}>
            ...
            {open && (
                <div className="absolute right-0 z-20 mt-2 w-80 rounded-xl border border-zinc-800 bg-black shadow-xl">
                    <div className="p-2 border-b border-zinc-800">
                        <input
                            value={q}
                            onChange={(e) => setQ(e.target.value)}
                            placeholder="Search GIFs"
                            className="w-full rounded-lg bg-zinc-950 px-3 py-2 text-sm outline-none border border-zinc-800 focus:border-zinc-600"
                        />
                    </div>
                    <div className="grid max-h-72 grid-cols-3 gap-1 overflow-auto p-2">
                        ...
                        {items.length === 0 && (
                            <div className="col-span-3 p-4 text-center text-sm text-zinc-500">
                                No GIFs
                            </div>
                        )}
                    </div>
